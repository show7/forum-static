...
export default class WorkItem extends React.Component<any,any> {
  ...
  render() {
    const { headPic, role, upName, upTime, content, onEditClick, onShowClick, signature, title, requestCommentCount, submitId, needFeedback } = this.props;
        ...
        return (
          <div className="controlContainer">
            {!request && requestCommentCount != null && requestCommentCount > 0 ?
              <span className="show" style={{marginRight:5}} onClick={()=>this.click(submitId)}>求点评</span>: null}
            {request || (requestCommentCount != null && requestCommentCount === 0) ?
              <span className="show disabled" style={{marginRight:5}}
                    onClick={()=>this.click(submitId)}>求点评</span>: null}
    ...
    return (
      <div className="workItemContainer">
        ...
        }
        {
          needFeedback &&
          <div className="requestComment">
            <img height={26} width={26} src="https://static.iqycamp.com/pen-fnxlqt5b.png"/>
          </div>
        }
        ...
        />
        <Alert title='操作确认' content={`当前小课还剩${requestCommentCount}次请求教练点评的机会，确定要在这次使用吗？`} open={alert}
               actions={actions}/>
      </div>
    )
  }
